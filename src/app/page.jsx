...
export default function Home() {
  ...
  useEffect(() => { setTimeout(() => setLd(true), 1800); }, []);
  useEffect(() => {
  ...
  return (<>
    ...
    <div style={{position:'fixed',top:0,left:0,width:'100%',height:'100%',background:'#14161B',zIndex:10000,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',opacity:ld?0:1,visibility:ld?'hidden':'visible',transition:'opacity 1s cubic-bezier(0.16,1,0.3,1),visibility 1s'}}>
      <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:'clamp(28px,5vw,48px)',fontWeight:300,color:'#F7F7F4',letterSpacing:'0.08em',textTransform:'uppercase'}}>Umbrella Realty Group</div>
      <div style={{fontFamily:'DM Mono,monospace',fontSize:'9px',letterSpacing:'0.3em',textTransform:'uppercase',color:'#B4975A',opacity:0.6,marginTop:12}}>The Kollective Hospitality Group</div>
    </div>
    ...
    <nav style={{position:'fixed',top:0,left:0,width:'100%',zIndex:1000,padding:'20px clamp(20px,4vw,80px)',display:'flex',alignItems:'center',justifyContent:'space-between',background:sc?'rgba(10,10,14,0.92)':'transparent',backdropFilter:sc?'blur(16px)':'none',transition:'background 0.4s'}}>
      <a href="#" style={{textDecoration:'none'}}><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:'clamp(14px,1.6vw,20px)',fontWeight:500,letterSpacing:'0.08em',textTransform:'uppercase',color:'#F7F7F4'}}>Umbrella Realty Group</div></a>
    ...
    <section style={{position:'relative',height:'100vh',overflow:'hidden',display:'flex',flexDirection:'column',justifyContent:'center',background:'#14161B'}}>
      <div style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',background:'linear-gradient(135deg,#14161B 0%,rgba(255,255,255,0.03) 50%,#14161B 100%)'}} />
        <img src="https://dzlmtvodpyhetvektfuo.supabase.co/storage/v1/object/public/brand-graphics/dr_dorsey/website/garden-district.jpg" alt="" style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',objectFit:'cover',opacity:0.3,animation:'slowZoom 20s ease-in-out infinite alternate'}} />
      <div style={{position:'relative',zIndex:3,maxWidth:620,padding:'0 clamp(20px,4vw,80px)'}}>
        <div style={{fontFamily:'DM Mono,monospace',fontSize:'clamp(8px,0.7vw,10px)',letterSpacing:'0.35em',textTransform:'uppercase',color:'#B4975A',marginBottom:20,animation:'fadeUp 0.8s cubic-bezier(0.16,1,0.3,1) 1.8s both'}}>Umbrella Realty Group × The Kollective</div>
        <h1 style={{fontFamily:'Cormorant Garamond,serif',fontSize:'clamp(32px,6vw,80px)',fontWeight:300,lineHeight:1.1,letterSpacing:'-0.02em',color:'#F7F7F4',margin:0,animation:'fadeUp 1s cubic-bezier(0.16,1,0.3,1) 2s both'}}>Buy. Sell. Invest.<br />One team.<br />Every <em style={{fontStyle:'italic',color:'#2C5F2D'}}>market.</em></h1>
        <p style={{fontSize:'clamp(13px,1.2vw,16px)',color:'rgba(247,247,244,0.45)',lineHeight:1.7,maxWidth:440,marginTop:24,animation:'fadeUp 0.8s cubic-bezier(0.16,1,0.3,1) 2.2s both'}}>Residential and commercial real estate. Buying, selling, leasing, investment consulting, and property management.</p>
        <a href="#services" style={{display:'inline-block',fontFamily:'DM Mono,monospace',fontSize:'clamp(8px,0.7vw,10px)',letterSpacing:'0.25em',textTransform:'uppercase',color:'#14161B',background:'#B4975A',padding:'14px 40px',textDecoration:'none',marginTop:40,animation:'fadeUp 0.8s cubic-bezier(0.16,1,0.3,1) 2.4s both'}}>Explore</a>
      </div>
      <div style={{position:'absolute',bottom:0,left:0,width:'100%',height:'2px',background:'linear-gradient(90deg,transparent,#B4975A,transparent)',zIndex:3,opacity:0.4}} />
    </section>
    ...
    {/* CINEMATIC BREAK */}
    <div style={{position:'relative',height:'clamp(200px,30vw,350px)',overflow:'hidden',background:'#14161B'}}>
      <div style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',background:`linear-gradient(180deg,#14161B 0%,transparent 30%,transparent 70%,#14161B 100%)`,display:'flex',alignItems:'center',justifyContent:'flex-start',padding:'0 clamp(40px,6vw,120px)'}}>
        <div style={{position:'relative',overflow:'hidden'}}>
      <img src="https://dzlmtvodpyhetvektfuo.supabase.co/storage/v1/object/public/brand-graphics/dr_dorsey/website/luxury-venue.jpg" alt="" style={{position:'absolute',top:0,left:0,width:'100%',height:'120%',objectFit:'cover',opacity:0.5}} />
      <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:'clamp(20px,3.5vw,48px)',fontWeight:300,fontStyle:'italic',color:'#B4975A',maxWidth:500,lineHeight:1.3}}>&ldquo;Real Estate Services — built different.&rdquo;</div>
      </div>
    </div>

    {/* ABOUT */}
    <section id="about" style={{padding:'clamp(80px,10vw,140px) clamp(20px,4vw,80px)',background:'#F7F7F4',color:'#14161B'}}>
      <div style={{maxWidth:1400,margin:'0 auto'}}>
        <div ref={addR} style={{opacity:0,transform:'translateY(35px)',transition:'opacity 0.8s cubic-bezier(0.16,1,0.3,1),transform 0.8s cubic-bezier(0.16,1,0.3,1)'}}>
          ...
          <p style={{fontSize:'clamp(14px,1.3vw,18px)',color:'rgba(20,22,27,0.5)',lineHeight:1.8,maxWidth:600}}>Umbrella Realty Group operates under The Kollective Hospitality Group — a multi-brand enterprise spanning events, food & beverage, museums, consumer products, technology, and wellness across 8 cities. Every division benefits from shared infrastructure, relationships, and operational excellence.</p>
        </div>
      </div>
    </section>

    {/* CONNECT */}
    <section id="connect" style={{padding:'clamp(80px,10vw,140px) clamp(20px,4vw,80px)',background:'#14161B'}}>
      <div style={{maxWidth:900,margin:'0 auto',textAlign:'center'}}>
        <div ref={addR} style={{opacity:0,transform:'translateY(35px)',transition:'opacity 0.8s cubic-bezier(0.16,1,0.3,1),transform 0.8s cubic-bezier(0.16,1,0.3,1)'}}>
          <div style={{width:40,height:1,background:'#B4975A',margin:'0 auto 16px'}} />
          ...
          <a href="mailto:thedoctordorsey@gmail.com?subject=Umbrella Realty Group Inquiry" style={{fontFamily:'DM Mono,monospace',fontSize:'clamp(8px,0.7vw,10px)',letterSpacing:'0.2em',textTransform:'uppercase',color:'#14161B',background:'#B4975A',padding:'14px 36px',textDecoration:'none',display:'inline-block'}}>Get In Touch</a>
        </div>
      </div>
    </section>
    ...
    <footer className="ftr" style={{padding:'40px clamp(20px,4vw,80px)',borderTop:'1px solid rgba(247,247,244,0.06)',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#14161B'}}>
      <div style={{fontFamily:'DM Mono,monospace',fontSize:'clamp(7px,0.65vw,9px)',letterSpacing:'0.2em',color:'rgba(247,247,244,0.3)'}}>© 2026 Umbrella Realty Group — The Kollective Hospitality Group</div>
